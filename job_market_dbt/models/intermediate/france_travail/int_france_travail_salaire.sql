-- Normalisation et imputation des salaires annuels bruts.
-- Source  : int_france_travail_offres_data + modèles métier, contrat, expérience, localisation
-- Sortie  : une ligne par offre avec salaire_min, salaire_max (annuels) et statut_salaire.
--
-- Étape 1 — Normalisation : conversion Horaire/Mensuel → Annuel.
-- Étape 2 — Imputation : remplacement des NULL par la médiane (PERCENTILE_CONT 0.5)
--   calculée en cascade sur des partitions de plus en plus larges :
--   métier+expérience+département+contrat → ...+région → ...sans localisation → métier+contrat → contrat seul.
--   Cette cascade garantit toujours une valeur imputée, même pour les combinaisons rares.
...
-- Étape 1 : conversion en base annuelle. Seuil 10 000 € pour distinguer mensuel/annuel mal typé.
salaire_normalise as (
    select
        id,
        case
            when salaire_periodicite = 'Horaire' then salaire_min * 35 * 52
            when salaire_periodicite = 'Mensuel' and salaire_min <= 10000.0 then salaire_min * 12
            when salaire_periodicite = 'Mensuel' and salaire_min > 10000.0 then salaire_min
            when salaire_min = 0.0 then null
            else salaire_min
        end as salaire_min,
        case
            when salaire_periodicite = 'Horaire' then salaire_max * 35 * 52
            when salaire_periodicite = 'Mensuel' and salaire_max <= 10000.0 then salaire_max * 12
            when salaire_periodicite = 'Mensuel' and salaire_max > 10000.0 then salaire_max
